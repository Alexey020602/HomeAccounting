...
public static class SaveCheckHandler
{
    ...
    {
        // if (await repository.GetCheckByRequest(request.CreateGetCheckRequestFromSaveCheckRequest()) is not null)
        //     return null;
...
//                 new StoreCheckCommand.Product(p.Name, p.Quantity, p.Price, p.Sum, p.Subcategory, p.Category)
//             ).ToList()
//         );
//     }
// }

public static class StoreCheckCommandExtensions
{
    internal static AddCheckRequest ConvertToAddCheckRequest(this StoreCheckCommand storeCheckCommand) => new
    (
        storeCheckCommand.Login,
        storeCheckCommand.PurchaseDate,
        storeCheckCommand.Fn,
        storeCheckCommand.Fd,
        storeCheckCommand.Fp,
        storeCheckCommand.Sum,
        storeCheckCommand.Products.Select(ConvertToProduct).ToList()
    );

    private static AddCheckRequest.Product ConvertToProduct(this StoreCheckCommand.Product product) => new(
        product.Name, product.Quantity, product.Price, product.Sum, product.Subcategory, product.Category);
}
...
public static class StoreCheckHandler
{
    ...
    {
        throw new TestException("Test in store");
